...
async def show_grades(message: types.Message, state: FSMContext):
    ...
    grades = await get_my_grades_from_db(student_id=student.id)
...
def show_grades_handlers(dp: Dispatcher):
    dp.register_message_handler(show_grades, commands="show_grades", state="*")
    dp.register_message_handler(show_grades, Text(equals="Отправленные оценки", ignore_case=True), state="*")

    dp.register_message_handler(back_menu, Text(equals="Назад", ignore_case=True), state="*")
